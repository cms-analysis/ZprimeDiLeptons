...
//                      04/05/2016                             = 
//==============================================================
#define ZprimeMuMuPatMiniAod_cxx
#include "ZprimeMuMuPatMiniAod.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include "TFile.h"
...
void ZprimeMuMuPatMiniAod::Loop(TString outfilename )
{
  ...
  h1_ZprimeRecomass20_           = new TH1F("ZprimeRecomass20","",50,0.0,1000.0);
...
//                                                   -  
//----------------------------------------------------
float ZprimeMuMuPatMiniAod::delR(float eta1,float phi1,float eta2,float phi2){
  float mpi=3.14;
...
bool ZprimeMuMuPatMiniAod::GenRecoMatchMu(float RecoEta1,float RecoPhi1){
  int NbHEEPele = 0;
  unsigned iflag = -10;
...
bool ZprimeMuMuPatMiniAod::SelectFirstMuon(float &pTmuon1,float &Enmuon1,float &Etamuon1,
...
bool ZprimeMuMuPatMiniAod::SelectSecondMuon(int ChargeMu1,unsigned FlagMu1,float pTmuon1,float Etamuon1,float Phimuon1,
...
void ZprimeMuMuPatMiniAod::PlotRecoInfo(float CosmicMuonRejec, float vertexMassMu,float MassGenerated,
  ...
  */
  ...
  //if( vertexMassMu > 3250 && vertexMassMu < 3750.0 ){h1_MassResultionEBEB8_->Fill((vertexMassMu-MassGenerated)/MassGenerated,weight);}
  //if( vertexMassMu > 3750 && vertexMassMu < 4250.0 ){h1_MassResultionEBEB9_->Fill((vertexMassMu-MassGenerated)/MassGenerated,weight);}
}
//===================== Methode to calculate the mass ========================
float ZprimeMuMuPatMiniAod::Mass(float Pt1,float Eta1,float Phi1,float En1,
...
void ZprimeMuMuPatMiniAod::PickThehighestMass(float &vtxHighestMass,float &vtxHighestChi2,int EvtNb)
{
  float Massinv  = -10.0;
  unsigned iflag = -10;
  vtxHighestMass = -10.0;
...
double ZprimeMuMuPatMiniAod::ThreeDangle(float pxMu1,float pyMu1,float pzMu1,float pMu1,
...
bool ZprimeMuMuPatMiniAod::SelectFirstGenMu(float &ETMu1,float &PhiSCMu1,
...
bool ZprimeMuMuPatMiniAod::SelectSecondGenMu(unsigned GenFlag1,float ETMu1,float &ETMu2,float &PhiSCMu2,
...
void ZprimeMuMuPatMiniAod::PlotGenInfo(float ZprimeGenMass,float EtaGenMu1,float EtaGenMu2,float PtGenMu1,
...
//                                                   -  
//----------------------------------------------------
void ZprimeMuMuPatMiniAod::MuonPassingID(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
...
void ZprimeMuMuPatMiniAod::PlotPterror(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAod::PlotNbTrackLayers(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAod::PlotNBValidPixelHits(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAod::PlotNbValidMuonHits(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAod::PlotNbMatchedStations(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAod::PlotTrackiso(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAod::PlotAbsDxy(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAod::PlotPtTuneP(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       Mu_absdxyTunePMuonBestTrack->at(i) < 0.2 &&
...
void ZprimeMuMuPatMiniAod::plotAllHighPtMuonsID(){
  MuonPassingID();
  PlotPterror();
  PlotNbTrackLayers();
...
void ZprimeMuMuPatMiniAod::MuonPassingNewID(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAod::MuonPassingTightID(){
  for(unsigned i=0; i<Mu_nbMuon->size(); i++){
    if(fabs(Mu_etaTunePMuonBestTrack->at(i)) < 2.4 &&
       Mu_isGlobalMuon->at(i) == 1 &&
       (Mu_ptTunePMuonBestTrack->at(i) > 53.0 && Mu_ptTunePMuonBestTrack->at(i) < ptEffCut) &&
...
void ZprimeMuMuPatMiniAod::CosThetaCollinSoper(float Et1,float Eta1,float Phi1,float En1,
...
void ZprimeMuMuPatMiniAod::PrintEventInformation(int runNumber, int lumiNumber, int eventNumber,
...
//                                                   -  
//----------------------------------------------------
bool ZprimeMuMuPatMiniAod::isPassHLT(){ 
  int nbMatch = 0;
...
bool ZprimeMuMuPatMiniAod::RecoHLTMuonMatching(float RecoEta,float RecoPhi){
  int nbMatch = 0;
